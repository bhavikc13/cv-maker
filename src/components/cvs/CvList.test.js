...
test("remove cv check", async (done) => {
  const history = createMemoryHistory();
  renderWithAll(<App />, history);
  const emailInputArea = screen.getByTestId("emailInputId");
  fireEvent.change(emailInputArea, {
  ...
  await new Promise((res) =>
    setTimeout(() => {
      expect(screen.getByTestId("removecv-testTestId")).toBeTruthy();
      res();
    }, 20000)
  );

  fireEvent.click(screen.getByTestId("removecv-testTestId"));
  await new Promise((res) =>
    setTimeout(() => {
      expect(screen.queryByTestId("cv-test")).not.toBeInTheDocument();
      res();
    }, 20000)
  );
  done();
