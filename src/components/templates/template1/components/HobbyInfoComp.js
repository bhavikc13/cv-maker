...
const HobbyInfoComp = (props) => {
  const [state, setState] = useState({ title: "", isLoading: false });
  props.updatePrevUrl(window.location.pathname);
  const [blocks, setBlocks] = useState(props.orderOfHobbyBlocks);
  useEffect(() => {
    setBlocks(props.orderOfHobbyBlocks);
  }, [props.orderOfHobbyBlocks]);
  useEffect(() => {
    props.updateOrderOfHobbyBlocks(blocks, props.auth.uid, props.cvid);
  }, [blocks]);
  ...
  const [, drop] = useDrop({ accept: "hobbyBlock" });
...
export default connect(mapStateToProps, mapDispatchToProps)(HobbyInfoComp);
